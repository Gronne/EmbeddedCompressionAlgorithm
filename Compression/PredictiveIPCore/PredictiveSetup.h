...
class PredictiveSetup : public ICompressionSetup<SensorT, ModelT>, public sc_module
{
public:
	//
	sc_in<bool> inClk;
	...
	PredictiveSetup(sc_module_name name):sc_module(name) {
		SC_CTHREAD(setupThread, inClk.pos());
		//reset_signal_is(inReset, true);
	};
	SC_HAS_PROCESS(PredictiveSetup);
	...
	void setupThread() {
		...
		while (true) {
			SensorT data;
			do {
				wait(inSensorDataReady.read() == true);
				data = inSensorData->read();
				setupDataBuffer.push(data);
				status = setup(data);
			} while (status == false);


			outModelCoefficientData.write(_coefficients);
			outModelReady.write(true);
			/*
			while (setupDataBuffer.size() > 0)
			{
				data = setupDataBuffer.front();
				setupDataBuffer.pop();
				outSensorData.write(data);
				out
	...
	bool setup(SensorT dataPoint) {
		//Add new datapoint of periodeData
